...
CORS уже включён. Фронт должен открываться по **http** (не как файл `file://`).

Запуск фронта (из папки `frontend`):

```
python -m http.server 5173 --bind 127.0.0.1
```

Открыть: http://localhost:5173 или http://127.0.0.1:5173 (адрес в браузере и в CORS должны совпадать).

Если в браузере «Failed to fetch», а Swagger на :8080 работает:

1. Перезапустите бэкенд после правки `.env`.
2. В `.env` укажите оба origin (см. `.env.example`):

```
CORS_ORIGINS=http://localhost:5173,http://127.0.0.1:5173
```

3. Не открывайте `index.html` двойным кликом — только через http.server.
4. API во фронте: `http://localhost:8080/api` (порт как в `PORT` бэкенда).
